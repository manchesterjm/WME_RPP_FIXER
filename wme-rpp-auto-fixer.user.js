...
(function() {
    ...
    let AddAlternateStreet = null;  // legacy action, late-bound in initCityFix
    //
    // NOTE on SDK migration: WME logs a deprecation warning on the legacy
    // AddAlternateStreet action and points to `sdk.DataModel.Segments.addAlternateStreet`.
    // However, the SDK method requires a pre-existing `streetId` — it does NOT
    // find-or-create a street for (name, cityId) like the legacy action does.
    // Using the SDK properly would require a separate street-creation call
    // (sdk.DataModel.Streets.* — surface not yet verified), so we stay on the
    // legacy action until that API is worked out. Functionally equivalent.
    ...
    function initCityFix() {
        ...
        try {
            ...
            console.warn('🏠 city-fix: AddAlternateStreet action class unavailable — segment edits disabled');
        }
        Promise.all([
    ...
    function cityClassify(rpp) {
        ...
        if (!streetName || !AddAlternateStreet) {
    ...
    function applyCityFix(rpp, plan, updateProps) {
        ...
        if (plan.type === 'add_alt') {
            if (!AddAlternateStreet) {
                console.warn('🏠 city-fix: AddAlternateStreet unavailable, cannot add alt to segment', plan.segment.attributes.id);
                return null;
            }
            try {
