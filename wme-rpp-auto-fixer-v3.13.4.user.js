...
(function() {
    ...
    console.log('Script loaded: WME RPP Auto-Fixer v3.13.4 - Auto-reset stats when starting scan');
    ...
    function fixRPP(rpp, needsEntryPoint, needsLockFix, targetLockRank, UpdateObject) {
        try {
            ...
            // FIX v3.13.3: Build a SINGLE UpdateObject with all properties that need updating
            // This prevents WME from rejecting/conflicting multiple UpdateObject calls on same venue
            const updateProps = {};
            let navPoint = null;

            // Prepare entry/exit point if needed
            if (needsEntryPoint) {
                try {
                    ...
                    navPoint = new NavigationPoint(geoJSONPoint);
                    updateProps.entryExitPoints = [navPoint];

                    console.log(`✅ Will add entry point to: ${address}`);
                } catch (err) {
            ...
            // Prepare lock level if needed
            if (needsLockFix) {
                updateProps.lockRank = targetLockRank;
                const displayLevel = targetLockRank + 1;
                console.log(`✅ Will set lock level ${displayLevel} for: ${address}`);
            }

            // Queue a SINGLE update action with all properties
            // This is critical - WME may reject or conflict multiple UpdateObject calls on same venue
            W.model.actionManager.add(new UpdateObject(rpp, updateProps));
    ...
    function startScanning() {
        try {
            ...
            // v3.13.4: Reset ALL session statistics when starting a new scan
            // This ensures each scan starts fresh without accumulated data from previous scans
